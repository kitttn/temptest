...
When using a Jetson TK1 as an AP, you have the option of [loading a pre-built flash image](https://github.com/projectara/Android-wiki/wiki/Build-and-Boot-Instructions-for-Jetson-reference-platform).  With this image flashed to your Jetson TK1, ready-to-use Greybus kernel modules are available in the /lib/modules directory.
...
3. Load the greybus, gb-phy, gb-es1 modules (in that order) into the AP's kernel.  When using the [pre-built Android image for the Jetson TK1](https://github.com/projectara/Android-wiki/wiki/Build-and-Boot-Instructions-for-Jetson-reference-platform), you'll need to run `su` and `cd` to the modules' location:
